...
#ifndef minMeetingRooms_hpp
#define minMeetingRooms_hpp
...
using namespace std;
class Solution {
    int _ifOverlap(vector<int> range, vector<int> tp){
        int top=0;
        int bottom=INT_MAX;
        for(int i=0; i<range.size(); i +=2){
            std::cout<<"range[i]: "<<range[i]<<", range[i+1]: "<<range[i+1]<<std::endl;
            if(!(tp[0] >= range[i+1] || tp[1] <= range[i])){
                std::cout<<"collision"<<std::endl;
                return -1;
            }else{
                if(tp[0] >= range[i+1]){
                    top = range[i+1] > top ? range[i+1] : top;
                    std::cout<<"top: "<<top<<std::endl;
                }else{
                    bottom = range[i] < bottom ? range[i] : bottom;
        ...
        return (bottom - top) - (tp[1]-tp[0]);
    }
    bool _findOverlaps(vector<vector<int>> & ranges, vector<int> tp){
        int min=INT_MAX;
        int minI=-1;
        int i = 0;
        for(auto &range: ranges){
            std::cout<<"i: "<<i<<std::endl;
            int dist = _ifOverlap(range, tp);
        ...
        if(min != INT_MAX){
            std::cout<<"inserting into existing i: "<<minI<<std::endl;
            ranges[minI].push_back(tp[0]);
            ranges[minI].push_back(tp[1]);
            return false;
        }
        return true;
    }
public:
    int minMeetingRooms(vector<vector<int>>& intervals) {
        ...
        for(auto &tp: intervals){
            std::cout<<"tp[0]: "<<tp[0]<<", tp[1]: "<<tp[1]<<std::endl;
            bool overlap = _findOverlaps(ranges, tp);
...
void test1(){
    ...
    std::cout<<s.minMeetingRooms(intervals)<<std::endl;
}
void test2(){
    Solution s;
