...
//  leetcode
//
//  Created by Prince Jain on 6/14/19.
...
struct Node{
    ...
    int pathCost;
    int cost;
    Node(string s):val(s),nb(0),pathCost(INT_MAX),cost(INT_MAX){}
};
typedef map<string,pair<vector<int>,int>> graph;
class Solution {
    int _stringDist(string s1, string s2){
        if(s1 == s2)
            return 0;
        int dist=0;
        int i=0;
    ...
    Node* _createGraphNodes(vector<string> &wordList, string beginWord){
        Node* found=nullptr;
        vector<Node*> nodevec;
        for(auto &i:wordList){
...
//            std::cout<<"node: "<<nodevec[i]->val<<": ";
            for(int j=0; j<nodevec.size(); j++){
                if(_stringDist(nodevec[i]->val, nodevec[j]->val) == 1){
                    neighbours.push_back(nodevec[j]);
//                    std::cout<<nodevec[j]->val<<" ";
                }
            }
//            std::cout<<std::endl;
            nodevec[i]->nb = neighbours;
        }
        if(found != nullptr){
            return found;
        }
        ...
        vector<Node*> neighbours;
//        std::cout<<"beginword: "<<beginWord<<": ";
        for(int j=0; j<nodevec.size(); j++){
            if(_stringDist(beginWord, nodevec[j]->val) == 1){
                neighbours.push_back(nodevec[j]);
//                std::cout<<nodevec[j]->val<<" ";
            }
        }
//        cout<<endl;
    ...
    //DFS -  not giving correct result for long wordList.
    int _minPath(Node* node, int  pathCost, string &endWord){
        string word = node->val;
        //        std::cout<<"word: "<<word<<", wordCost: "<<g[word].second<<", pathCost: "<<pathCost<<std::endl;
        if(word == endWord){
            if(node->pathCost > pathCost){
                node->pathCost = pathCost;
                node->cost = pathCost;
//                std::cout<<"found shorter path: "<<endWord<<", pathCost: "<<pathCost<<std::endl;
                return pathCost;
            }
//            std::cout<<"found: "<<endWord<<", pathCost: "<<pathCost<<std::endl;
            
            return pathCost;
        }
        if(node->pathCost != INT_MAX && node->pathCost < pathCost){ // g[word].second != -1 &&
            if(node->cost == INT_MAX)
                return INT_MAX;
            else{
                //                std::cout<<"found longer path: "<<word<<", wordCost[index].second: "<<wordCost[index].second<<std::endl;
                
                return node->cost - node->pathCost + pathCost;
            }
        }
        if(node->pathCost != INT_MAX && node->pathCost >= pathCost){ // g[word].second != -1 &&
            if(node->cost == INT_MAX){
                //                std::cout<<"not found shorter path: "<<word<<", wordCost[index].second: "<<wordCost[index].second<<std::endl;
                node->pathCost = pathCost;
                
                return INT_MAX;
            }
            else{
                int ret =  node->cost - node->pathCost + pathCost;
                
                node->pathCost = pathCost;
                node->cost = ret;
//                std::cout<<"found shorter path: "<<word<<", node->cost: "<<node->cost<<std::endl;
        ...
        int minCost=INT_MAX;
        for(auto &i: node->nb){
            int cost = _minPath(i, pathCost+1, endWord);
            //            minCost = minCost < cost ? minCost : cost;
            minCost = minCost < cost ? minCost : cost;
        }
        node->cost = minCost;
        //        std::cout<<"word: "<<word<<", wordCost: "<<g[word].second<<", pathCost: "<<pathCost<<", returning: "<<minCost<<std::endl;
        return minCost;
    }
    //BFS
    int _minPathBFS(Node* node, string &endWord){
        queue<Node*> stepq;
        stepq.push(node);
        int depth = 0;
        while(!stepq.empty()){
            depth++;
//            std::cout<<"q size: "<<stepq.size()<<std::endl;
            int size = (int)stepq.size();
            for(int i=0; i<size; i++){
                Node* node = stepq.front();
//                std::cout<<node->val<<" ";
                node->pathCost = depth;
                stepq.pop();
                if(node->val == endWord){
...
//        Node * begin = _createGraphNodes(wordList, beginWord);
//        int cost = _minPathBFS(begin, endWord);//(begin,pathCost+1,endWord);
//        return cost;
...
//        }
        while(!q.empty()){
            ...
            std::cout<<"qsize: "<<qsize<<" ";
            for(int i=0; i<qsize; i++){
                string word = q.front();
//                if(q.front() == beginWord){
////                    done[word] = true;
//                }
                std::cout<<word<<", ";
                if(word == endWord){
                ...
                for(auto it= wordSet.begin(); it != wordSet.end(); it++){
                    if(_stringDist(*it, word) == 1 && *it != beginWord){ // && done[*it] == false
                        q.push(*it);;
                        wordSet.erase(it);
                    }
                }
